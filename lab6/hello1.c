...
int print_hello(void)
{
	...
	pr_debug("%s: Time before print: %lld ns\n", __func__, ktime_to_ns(event->time_before));
	...
	pr_debug("%s: Time after print: %lld ns\n", __func__, ktime_to_ns(event->time_after));

	/* Add to list */
	list_add_tail(&event->list, &event_list);
	
	pr_debug("%s: Event added to list\n", __func__);
...
{
	pr_debug("%s: Module initialization started\n", __func__);
	pr_info("hello1: module loaded\n");
	pr_debug("%s: Module initialization completed\n", __func__);
	return 0;
}
...
static void __exit hello1_exit(void)
{
	...
	pr_debug("%s: Starting module unload procedure\n", __func__);
	...
	list_for_each_entry_safe(event, tmp, &event_list, list) {
		...
		pr_debug("%s: time before: %lld ns, time after: %lld ns, duration: %lld ns\n",
			__func__,
	...
	pr_debug("%s: Finished cleaning up event list\n", __func__);
